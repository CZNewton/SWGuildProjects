...
<html>
    ...
    <body>
        ...
        <div class="col-xs-7 body">
            ...
            <c:forEach var="OrgItem" items="${OrgList}">
            
            <tr>
                <th>${OrgItem.orgID}</th>
                <th>${OrgItem.name}</th>
                <th>${OrgItem.alignment}</th>
                <th>${OrgItem.description}</th>
            <form method="GET" action='editOrg'>
                <input type="hidden" name="ID" value="${OrgItem.orgID}">
                <th><button class="btn btn-default">edit</button></th>
            </form>
            <form method="POST" action="removeOrg">
                <input type="hidden" name="ID" value="${OrgItem.orgID}">
